...
from .enhancebase_mask_head import enhanceBaseMaskHead
...
class PAM(nn.Module):
    ...
    def forward(self, x, y):
        ## The code here will be updated soon...
        #return torch.cat([groupxx, groupyy], dim=1)
        return 0
...
class enhanceCondInstBboxHead(FCOSHead):
    ...
    def _init_layers(self) -> None:
        ...
        self.cls_convs_all_levels = nn.ModuleList()
        for i in range(0, 3):
            ...
            convs_per_level.add_module(
                ...
                ConvModule(
                    ...
                    inplace=False,
                    bias=False))
            self.cls_convs_all_levels.append(convs_per_level)

        self.reg_convs_all_levels = nn.ModuleList()
        for i in range(0, 3):
            convs_per_level = nn.Sequential()
            convs_per_level.add_module(
                f'conv{i}',
                ConvModule(
                    ...
                    inplace=False,
                    bias=False))
            self.reg_convs_all_levels.append(convs_per_level)
    ...
    def get_box_feature(self,x):

        cls_feats = []
        reg_feats = []
        for i in range(3):
            _, _ ,cls_feat, reg_feat= super(FCOSHead, self).forward_single(x[i])
            cls_feats.append(cls_feat)
            reg_feats.append(reg_feat)
        cls_feat_outs = self.cls_convs_all_levels[0](cls_feats[0])
        target_h, target_w = cls_feat_outs.size()[2:]
        for i in range(1, 3):
            input_p = cls_feats[i]
            x_p = self.cls_convs_all_levels[i](input_p)
            h, w = x_p.size()[2:]
            factor_h = target_h // h
            factor_w = target_w // w
            assert factor_h == factor_w
            feature_per_level = aligned_bilinear(x_p, factor_h)
            cls_feat_outs = cls_feat_outs + \
                feature_per_level

        reg_feat_outs = self.reg_convs_all_levels[0](reg_feats[0])
        target_h, target_w = reg_feat_outs.size()[2:]
        for i in range(1, 3):
            input_p = reg_feats[i]
            x_p = self.reg_convs_all_levels[i](input_p)
            h, w = x_p.size()[2:]
            factor_h = target_h // h
            factor_w = target_w // w
            assert factor_h == factor_w
            feature_per_level = aligned_bilinear(x_p, factor_h)
            reg_feat_outs = reg_feat_outs + \
                            feature_per_level



        box_feature = torch.cat((cls_feat_outs, reg_feat_outs),dim=1)


        box_feature = self.conv_pred(box_feature)
    ...
    def predict_by_feat(self,
                        cls_scores: List[Tensor],
                        bbox_preds: List[Tensor],
                        score_factors: Optional[List[Tensor]] = None, ###score_factors means centerness
...
class enhanceMaskFeatModule(BaseModule):
...
@MODELS.register_module()
class enhanceCondInstMaskHead(enhanceBaseMaskHead):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.mask_feature_head = enhanceMaskFeatModule(**mask_feature_head)
        self.mask_feat_stride = self.mask_feature_head.mask_stride
        self.in_channels = self.mask_feature_head.out_channels + 16
        ...
        self.oc1 = Conv_BN_ACT(in_channels=36, out_channels=36, kernel_size=3, alpha_in=0.5, alpha_out=0.5, padding=1)
        self.oc2 = Conv_BN_ACT(in_channels=36, out_channels=36, kernel_size=3, alpha_in=0.5, alpha_out=0.5, padding=1)
        self.oc3 = Conv_BN_ACT(in_channels=36, out_channels=36, kernel_size=3, alpha_in=0.5, alpha_out=0.5, padding=1)
    ...
    def _init_layers(self) -> None:
        ...
        for i in range(self.num_layers):
            if i == 0:
                weight_nums.append((self.in_channels + 2 + 2) * self.feat_channels)
        ...
        # offset_weight_nums, offset_bias_nums = [], []
        # for j in range(1):
        #     offset_weight_nums.append(self.feat_channels*3*3*2)
        #     offset_bias_nums.append(2)
        # self.offset_weight_nums = offset_weight_nums
        # self.offset_bia_nums = offset_bias_nums

        # self.num_params = sum(weight_nums) + sum(bias_nums) + sum(offset_weight_nums) + sum(offset_bias_nums)
    ...
    def parse_dynamic_params(
        ...
        params_splits = list(
            torch.split_with_sizes(
                params, self.weight_nums + self.bias_nums, dim=1))   ### + self.offset_weight_nums + self.offset_bia_nums
        weight_splits = params_splits[:self.num_layers]
        bias_splits = params_splits[self.num_layers:]
        # bias_splits = params_splits[self.num_layers:self.num_layers*2]
        # offset_weight_splits = params_splits[self.num_layers*2:self.num_layers*2+1]
        # offset_bias_splits = params_splits[self.num_layers*2+1:]
        for i in range(self.num_layers):
            if i < self.num_layers - 1:
                ...
                weight_splits[i] = weight_splits[i].reshape(
                                        num_insts * 16 , -1, 1, 1)
                bias_splits[i] = bias_splits[i].reshape(num_insts * 16)
            else:
        ...
        # for i in range(1):
        #     offset_weight_splits = offset_weight_splits[i].reshape(num_insts *2,-1,3,3)
        #     offset_bias_splits = offset_bias_splits[i].reshape(num_insts*2)
        # return weight_splits, bias_splits, offset_weight_splits,offset_bias_splits
    ...
    def forward(self, x: tuple, positive_infos: InstanceList, box_feature) -> tuple:
        ...
        # box_feature = self.ema(box_feature)
        # mask_feats = torch.cat((mask_feats,box_feature),dim=1)
        # mask_feats = mask_feats + box_feature
        return multi_apply(self.forward_single, mask_feats, positive_infos, box_feature)

    def forward_single(self, mask_feat: Tensor,
                       positive_info: InstanceData, box_feature) -> Tensor:
        ...
        mask_head_inputs_pre1 = torch.cat([rel_coords, mask_feat], dim=1)


        box_feature = box_feature[None].repeat(num_inst, 1, 1, 1)
        # mask_head_inputs_pre2 = torch.cat([rel_coords, box_feature], dim=1)
        mask_head_inputs = self.PAM(box_feature,mask_head_inputs_pre1)

        # from .visulization_fature_map import draw_feature_map1
        # draw_feature_map1(mask_head_inputs, img_path = batch_img_metas['img_path'])
        ...
        mask_head_inputs_h, mask_head_inputs_l = self.oc1(mask_head_inputs)
        mask_head_inputs_h, mask_head_inputs_l = self.oc2((mask_head_inputs_h, mask_head_inputs_l))
        mask_head_inputs_h, mask_head_inputs_l = self.oc3((mask_head_inputs_h, mask_head_inputs_l))

        mask_head_inputs_l = mask_head_inputs_l.reshape(1, -1, int(H / 2), int(W / 2))
        mask_head_inputs_h = mask_head_inputs_h.reshape(1, -1, H, W)
        mask_head_inputs = mask_head_inputs.reshape(1, -1, H, W)

        mask_preds_h = self.dynamic_conv_forward(mask_head_inputs_h, weights, biases, num_inst)
        mask_preds_l = self.dynamic_conv_forward(mask_head_inputs_l, weights, biases, num_inst)
        mask_preds = self.dynamic_conv_forward(mask_head_inputs, weights, biases, num_inst)

        mask_preds_l = aligned_bilinear(mask_preds_l, 2)
        mask_preds_hl = mask_preds_l + mask_preds_h
        # mask_head_inputs_hl = self.relu(mask_head_inputs_hl)
        mask_preds = mask_preds + mask_preds_hl


        # rel_coords_pos = abs(rel_coords).max() - abs(rel_coords)
        # Threshold = abs(rel_coords).max() - 0.125
        # rel_coords_pos[rel_coords_pos > Threshold] = abs(rel_coords).max()
        # rel_coords_pos[rel_coords_pos < Threshold] = 0
        # rel_coords_pos = rel_coords_pos / rel_coords_pos.max()

        # point_grid = torch.zeros([num_inst, 2, H, W], dtype=torch.float,device=mask_feat.device)
        # pos_points = pos_points / 8.0
        # for i in range(num_inst):
        #     x = pos_points[i, 0].int()
        #     y = pos_points[i, 1].int()
        #     point_grid[i, 0, x - 1, :] = 1
        #     point_grid[i, 1, :, y - 1] = 1

        # pos_centerness = pos_centerness.repeat(1,2*H*W).reshape(num_inst,-1,H,W)
        # generated_offset = F.conv2d(mask_preds, offset_weight, bias=offset_biaes, stride=1, padding=1, groups=num_inst)
        # generated_offset = generated_offset.reshape(num_inst,2,H,W)
        # mask_preds = mask_preds.reshape(num_inst,-1,H,W)
        # mask_preds = self.deformable(mask_preds, generated_offset)
        # mask_preds = mask_preds.reshape(1,-1,H,W)
    ...
    def _get_targets_single(self, mask_preds: Tensor,
        ...
        gt_masks = gt_masks[:, start::self.mask_out_stride,
                            start::self.mask_out_stride]     # the size of gt_mask
