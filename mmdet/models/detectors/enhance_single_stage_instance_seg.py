...
class enhanceSingleStageInstanceSegmentor(BaseDetector):
    ...
    def _forward(self,
        ...
        if self.with_bbox:
            ...
            box_feature = self.bbox_head.get_box_feature(x)
            # offset = self.bbox_head.get_offset(x)
        # mask_head
        if positive_infos is None:
            mask_outs = self.mask_head.forward(x, box_feature)
        else:
            mask_outs = self.mask_head.forward(x, positive_infos, box_feature)
        outs = outs + (mask_outs, )
        return outs
    ...
    def loss(self, batch_inputs: Tensor, batch_data_samples: SampleList,
        ...
        mask_loss = self.mask_head.loss(
            x, batch_data_samples, positive_infos=positive_infos, box_feature = box_feature ,**kwargs)
        # avoid loss override
    ...
    def predict(self,
        ...
        results_list = self.mask_head.predict(
            x, batch_data_samples, box_feature = box_feature,  rescale=rescale, results_list=results_list)
