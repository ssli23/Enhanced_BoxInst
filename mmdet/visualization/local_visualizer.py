...
class DetLocalVisualizer(Visualizer):
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'masks' in instances:
            ...
            self.draw_binary_masks(masks, colors=(255,165,0), alphas=self.alpha)  ###(189, 252, 201)（219，112，147）
    ...
    def add_datasample(
        ...
        if draw_pred and data_sample is not None:
            ...
            if 'pred_instances' in data_sample:
                ...
                # print('pred_instances.masks : ', pred_instances.masks.shape)

                if len(pred_instances.masks) != 0:
                    pred_img_data = self._draw_instances(image, pred_instances, classes, palette)
                # pred_img_data = self._draw_instances(image, pred_instances,                            ####### new add
                #                                      classes, palette)
