...
class ResultVisualizer:
    ...
    def _save_image_gts_results(self,
        ...
        for performance_info in performances:
            ...
            if task == 'det':
                ...
                # gt_instances.masks = results[index]['gt_instances']['masks']
                ...
                pred_instances.scores = results[index]['pred_instances'][
                    'scores']
                pred_instances_masks = results[index]['pred_instances'][
                    'masks']

                # segms = mmcv.concat_list(masks)
                pred_instances_masks = mask_util.decode(pred_instances_masks)
                pred_instances_masks = pred_instances_masks.transpose(2, 0, 1)
                pred_instances.masks = pred_instances_masks
    ...
    def evaluate_and_show(self,
        ...
        if 'pred_panoptic_seg' in results[0].keys():
            ...
            good_samples, bad_samples, all_samples = self.detection_evaluate(
                dataset, results, topk=topk)
            self._save_image_gts_results(
                dataset, results, good_samples, good_dir, task='det')
            self._save_image_gts_results(
                dataset, results, bad_samples, bad_dir, task='det')
            self._save_image_gts_results(                                   #### new add
                dataset, results, all_samples, all_dir, task='det')         #### new add
        else:
    ...
    def detection_evaluate(self, dataset, results, topk=20, eval_fn=None):
        ...
        return good_mAPs, bad_mAPs, all_mAPs
...
def parse_args():
    ...
    parser.add_argument('--config', help='test config file path',default='/media/kb535/Data/work_dirs/codes&results/mycodes/configs/tumor/my_r101_fpn_ms_90k_lits.py')
    parser.add_argument(
        '--prediction_path', help='prediction path where test pkl result',default='/media/kb535/Data/work_dirs/codes&results/mycodes/tools/321.pkl')
    parser.add_argument(
        '--show_dir', help='directory where painted images will be saved',default='/media/kb535/Data/work_dirs/codes&results/mylitsresults')
